...
function Window({ nomeArquivo, onClose }) {
  ...
  useEffect(() => {
    ...
    import(`../../../componentes/dados/${nomeArquivo}.js`)
      .then((module) => {
    ...
    if (typeof codigo === "object") {
      return (
        <div className="codigo-objeto">
          <div>URL: {codigo.url}</div>
          <div>Headers: {JSON.stringify(codigo.headers, null, 2)}</div>
          <div>Body: {codigo.body}</div>
        </div>
      );
    }
